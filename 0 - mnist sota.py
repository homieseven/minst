...
def prepare_model_containers():
    ...
    mc = ModelContainer(model = model, description = "32(f3,1)/1024", text_color='b')
    ...
    model.add(layers.Activation("relu"))
    model.add(layers.Dense(10, activation='softmax'))
    mc = ModelContainer(model=model, description="32(f5,1)/1024", text_color='r')
    containers.append(mc)
    ...
    model.add(layers.Activation("relu"))
    model.add(layers.Dense(10, activation='softmax'))
    mc = ModelContainer(model=model, description="64(f3,1)/1024", text_color='y')
    containers.append(mc)

    model = models.Sequential()
    model.add(layers.Conv2D(64, (5, 5), input_shape=(28, 28, 1)))
    model.add(layers.Activation("relu"))
    model.add(layers.Flatten())
    model.add(layers.Dense(1024))
    model.add(layers.Activation("relu"))
    model.add(layers.Dense(10, activation='softmax'))
    mc = ModelContainer(model=model, description="64(f5,1)/1024", text_color='g')
    containers.append(mc)
