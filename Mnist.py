...
def prepare_model_containers():
    ...
    mc = ModelContainer(model=model, description="32(f5,1)/MP(2)/32(f3,1)/MP(2)/512", text_color='c')
    containers.append(mc)
    print(model.summary())

    model = models.Sequential()
    model.add(layers.Conv2D(32, (9, 9), input_shape=(28, 28, 1)))
    model.add(layers.Activation("relu"))
    model.add(layers.MaxPooling2D(pool_size=(2, 2)))
    ...
    model.add(layers.Activation("relu"))
    model.add(layers.Dense(10, activation='softmax'))
    mc = ModelContainer(model=model, description="32(f9,1)/MP(2)/32(f5,1)/MP(2)/512", text_color='r')
    containers.append(mc)
    print(model.summary())
