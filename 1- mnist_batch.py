...
def main():
    ...
    batches_results = np.zeros((len(batches),2))
    iter = 8
    ...
    for b, batch in enumerate(batches):
        ...
        for i in range(0, iter):
            ...
            history = model.fit(x=train_data, y=train_label, batch_size=batch, epochs=1,
                                validation_data=(val_data, val_label))

            result = model.evaluate(test_data, test_label)
            print('Batch size: {0}, iteration: {1}, results: {2}'.format(batch, i+1, result))
            array = np.array(result)
            results[i,:] = array
        #calculate average
        res = np.average(results,axis=0)
        batches_results[b,:] = res
    print(batches_results)

    plt.figure(1)
    plt.subplot(211)
    plt.title('Average Loss')
    plt.plot(batches, batches_results[:,0], 'b')

    plt.subplot(212)
    plt.title('Average accuracy')
    plt.plot(batches, batches_results[:,1],'r')
    plt.show()
